...
fn rfc3610_packet_vector_1() {
    ...
    let expected_ct: [u8; 23] = [
        0x58, 0x8C, 0x97, 0x9A, 0x61, 0xC6, 0x63, 0xD2, 0xF0, 0x66, 0xD0, 0xC2, 0xC0, 0xF9, 0x89, 0x80, 0x6D,
        0x5F, 0x6B, 0x61, 0xDA, 0xC3, 0x84,
    ];
    ...
    for aad in [&[][..], &[0xAA, 0xBB, 0xCC][..]] {
        for &len in &[0usize, 1, 15, 16, 17, 31, 32, 40] {
            ...
            assert_eq!(
                &data[..len],
                &plain[..len],
                "round-trip plaintext mismatch (len {len})"
            );
        }
    }
}
...
fn single_byte_tamper_rejected() {
    ...
    // Tamper the ciphertext.
    {
        ...
        d[5] ^= 0x01;
        assert!(
            !ccm.open(&key, &nonce, &aad, &mut d, &tag),
    ...
    // Wrong key.
    {
        ...
        wrong[15] ^= 0x01;
        assert!(
            !ccm.open(&wrong, &nonce, &aad, &mut d, &tag),
            "wrong key must fail"
        );
    }
