...
bootloader. Layout (docs/fota.md):
    ...
    0x00000  bootloader (from <boot.bin>, linked at 0x0800_0000; ≤32K)
    ...      0xFF padding  (incl. the state region 0x8000..0xB000 and the manifest region
             0xB000..0xB800, so a fresh boot reads "no swap" + "no manifest")
    0x0B800  app (from <app.bin>, linked into ACTIVE at 0x0800_B800)

The DFU slot (0x1_D000) is left out of the image; the app erases + writes it when it
...
ACTIVE = 0xB800  # ACTIVE offset = 0x0800_B800 - flash base 0x0800_0000 (docs/fota.md)
