...
Build full-flash with `just flash example <app>`; build FOTA-capable with `just flash-fota <app>`
...
just run example fota_stage               # → *** ALL PASS ***
...
- **Host tools:** `tools/fota-sign/` (signer), `tools/fota_merge.py` (merge). `tower fota serve`
  (in `tower-cli`). `just` recipes: `flash-fota`, `fota-sign`, `fota-image`, `fota-update`.
